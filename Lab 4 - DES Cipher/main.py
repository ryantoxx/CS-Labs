...
class DesCipher:
    ...
    def calculate_Ri(Li_minus_1, Ri_minus_1, Round_Key):
        expanded_Ri = DesCipher.get_expansion(Ri_minus_1)
        print(f"Expanded Ri from 32 bits to 48 bits: {expanded_Ri}.")
        xor_operation = ''.join(str(int(a) ^ int(b)) for a, b in zip(Round_Key, expanded_Ri))
        print(f"The XOR operation between Round Key and the expanded Ri(48 bits): {xor_operation}.")
        S_box_result = DesCipher.get_box_substitution(xor_operation)
        print(f"The bits were ran through S-Boxes(32 bits): {S_box_result}.")
        P_box_mix = DesCipher.get_Pbox_permutation(S_box_result)
        print(f"Permutation of the 32-bit output: {P_box_mix}.")
